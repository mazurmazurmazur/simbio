...
fetch("http://mazurmazurmazur.pl/simbiocms/?rest_route=/wp/v2/article/"+dynamicContent) //only one entry in json file (WP REST)
      .then(res => res.json())
      .then(showProducts)
  }
  function showProducts(json) { 
    ...
    let acf=json.acf;
     ...
     for(let i = 0; i<thumbGallery.children.length; i++){
         ...
         let thumbJson = (eval("json.acf.image"+imageFetchHelper+".sizes.medium_large"));
